...
export const ProductEdit = () => {
  ...
  function handleCurrentProductData() {
    let productFind = allProducts?.find(
      (data: any) => data.userId == loggedInUser && data.name == productId
    );
    ...
    setImageUrl(productFind?.coverImage);
    setCategorySelected(productFind?.category)
  }
  ...
  useEffect(() => {
    if (allProducts?.length) handleCurrentProductData();
  }, [allProducts?.length]);
  ...
  const onSubmit = useCallback(() => {
    ...
    if (productExist && name!=productId) {
      setError("name", {
        type: "manual",
 ...
 let updatedProducts = allProducts?.map((data: any) => {
       if (productId == data.name && loggedInUser == data.userId) {
         return Product;
       }
       return data;
     });
  ...
  return (
    <div>
      <PageHeading backButton={true} text="Edit Product" />
      <div className="w-full flex items-center justify-center ">
        ...
        >
          ...
          <select  value={categorySelected} onChange={(e:any)=> setCategorySelected(e.target.value)} name="category" className="p-2 rounded mt-1 mb-4">
            {
              userCategories?.map((data:any)=> 

...
import { RunUpdateProductsNameTransaction } from "../../redux/reducers/ProductReducer";
...
export const CategoryEdit = () => {
  ...
  function handleCurrentCategoryData() {
    let categoryFind = allCategories?.find(
      (data: any) => data.userId == loggedInUser && data.name == categoryId
    );
    ...
    setImageUrl(categoryFind?.coverImage);
  }
  ...
  useEffect(() => {
    if (allCategories?.length) handleCurrentCategoryData();
  ...
  const onSubmit = useCallback(() => {
    ...
    if (categoryExist && categoryId != name) {
      setError("name", {
        type: "manual",
    ...
    if(categoryId!=name){
      dispatch(RunUpdateProductsNameTransaction({ newName : name, oldName : categoryId }));
    }
    let updatedCategories = allCategories?.map((data: any) => {
      if (categoryId == data.name && loggedInUser == data.userId) {
        return Category;
      }
      return data;
    });
  ...
  return (
    <div>
      <PageHeading backButton={true} text="Edit Category" />
